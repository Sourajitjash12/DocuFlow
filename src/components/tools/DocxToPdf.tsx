...
export const DocxToPdf: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    setProgress(20);
    setProgressText('Uploading Word document to conversion API...');
    ...
    try {
      ...
      setProgress(60);
      setProgressText('Building PDF vector stream on server...');
      ...
      if (!response.ok) {
        ...
        throw new Error(errJson.error || 'Word to PDF conversion failed.');
      }

      const blob = await response.blob();
      const disposition = response.headers.get('Content-Disposition') || '';
      const filenameMatch = disposition.match(/filename="(.+)"/);
      const fileName = filenameMatch ? filenameMatch[1] : item.name.replace(/\.docx$/i, '.pdf');
      ...
      {status === 'idle' && (
        <div className="space-y-6">
          <FileUploadZone
            ...
            acceptedTypesLabel="Word Documents (.docx)"
            maxFiles={1}
            files={files}
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 space-y-4 shadow-subtle">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-semibold text-slate-900">Word Document Selected</h3>
                </div>
      ...
      {status === 'processing' && (
        <div className="rounded-xl border border-slate-200 bg-white p-8 text-center space-y-6 shadow-subtle">
          ...
          <div className="space-y-2">
            <h3 className="text-base font-semibold text-slate-900">Converting Word Document...</h3>
      ...
      {status === 'success' && result && (
        <div className="rounded-xl border border-emerald-200 bg-white p-8 space-y-6 shadow-card">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-slate-900">Converted to PDF!</h3>
              <p className="text-xs text-slate-500">Your Word document was converted cleanly.</p>
            </div>
          </div>
